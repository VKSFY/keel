"""`pyge` command-line entry points: new / run / build.

This module never imports the rest of pyge at the top level so the CLI
...
def cmd_new(project_name: str) -> int:
    ...
    if root.exists():
        print(f"[pyge] error: {project_name!r} already exists", file=sys.stderr)
        return 1
    ...
    print(f"[pyge] created project at {project_name}/")
    return 0
...
def cmd_run(entry: str = "main.py", *, watch_dir: str = ".") -> int:
    ...
    if not Path(entry).exists():
        print(f"[pyge] error: entry {entry!r} not found", file=sys.stderr)
        return 1

    print(f"[pyge] running {entry}")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pyge",
        description="PyGE developer CLI: scaffold, run, and (later) build projects.",
    )
...
def main(argv: list[str] | None = None) -> int:
    """argparse dispatch for `pyge <subcommand>`. Returns the exit code."""
