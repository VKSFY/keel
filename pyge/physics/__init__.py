"""Physics bridges for PyGE — pymunk for 2D, pybullet for 3D.
...
def setup_physics_2d(
    ...
    existing = getattr(app, "_pyge_physics_2d", None)
...
def setup_physics_3d(
    ...
    existing = getattr(app, "_pyge_physics_3d", None)
    if existing is not None:
        return existing
