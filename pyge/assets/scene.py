...
classes carrying the `__pyge_component__` metadata attached by Phase 1's
...
def _discover_components() -> dict[str, type]:
    """Walk every loaded module and collect classes carrying __pyge_component__."""
    found: dict[str, type] = {}
    for mod in list(sys.modules.values()):
        ...
        for obj in members.values():
            if not isinstance(obj, type):
                continue
            if getattr(obj, "__pyge_component__", None) is None:
                continue
            name = obj.__name__
