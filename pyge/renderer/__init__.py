"""2D renderer for PyGE: textures, shaders, sprite batching, tilemaps, and a setup helper."""
...
def setup_renderer_2d(app: Any) -> "Renderer2DSetup":
    ...
    existing = getattr(app, "_pyge_renderer_2d", None)
    ...
    atlas.add_texture("__pyge_default_white__", ctx.texture((1, 1), 4, b"\xff\xff\xff\xff"))
    shader_cache = ShaderCache()
    ...
    def render_2d(
        ...
        if getattr(app, "_pyge_tilemap", None) is None:
...
def setup_tilemap(
    ...
    existing: TilemapSetup | None = getattr(app, "_pyge_tilemap", None)
    if existing is not None:
    ...
    renderer_setup: Renderer2DSetup = app._pyge_renderer_2d
